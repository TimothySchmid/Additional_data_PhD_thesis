...
% ----------------------------------------------------------------------- %
  ...
  INPUT.datatype            = 'R1_ext_inc6_sumdiff_15-5';
  INPUT.colormap            = 'broc';
  INPUT.save                = 'yes';
  INPUT.xvectors            = 10;
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
  ...
  path_to_png = [path_to_experiment, '/png_Eyy_sum'];
...
X(1:xind_first + cut ,:) = NaN;
X(xind_last - cut:end,:) = NaN;
X(:,1:yind_first + cut)  = NaN;
X(:,yind_last - cut:end) = NaN;

Y(1:xind_first + cut ,:) = NaN;
Y(xind_last - cut:end,:) = NaN;
Y(:,1:yind_first + cut)  = NaN;
Y(:,yind_last - cut:end) = NaN;
...
% finite strain (Boutelier et al., 2019)
  Exx = 1/2 * (dudx + dudx + (dudx .* dudx + dvdx .* dvdx));
  Exy = 1/2 * (dudy + dvdx + (dudx .* dudy + dvdx .* dvdy));
  Eyx = 1/2 * (dvdx + dudy + (dvdy .* dvdx + dudy .* dudx));
  Eyy = 1/2 * (dvdy + dvdy + (dudy .* dudy + dvdy .* dvdy));
    ...
    pc = pcolor(X,Y,Eyy.*1e2);
    pc.FaceColor = 'interp';
    pc.EdgeColor = 'none';
    
    cb = colorbar;
    cb.FontSize = 16;
    ylabel(cb, 'Strain Eyy (%)')
    caxis([-1.5 1.5]*1e2)
    
    title(['Time: ' num2str((iRead+shift-1) * dt / 60) ' min'])
...
q = quiver(X(x_vec_array,y_vec_array),Y(x_vec_array,y_vec_array),...
           ...
           'color',[0.75 0.25 0.25],'LineWidth',2);
...
switch INPUT.save
    case 'yes'
        exportgraphics(hAx,[path_to_png,'/',INPUT.datatype,'_Eyy_',...
            num2str((iRead+shift-1) * dt / 60),' min.png'],'Resolution',600)
    case 'no'
    otherwise
