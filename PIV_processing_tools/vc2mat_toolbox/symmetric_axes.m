...
% INPUT
% ----------------------------------------------------------------------- %
  INPUT.experimentname      = 'R4';
  INPUT.datatype            = 'R4_short_inc5_diff_13-4';
...
if iRead == 1
   ...
   left_x  = xcoords(xind_first) - xcoords(1);
   right_x = xcoords(end) - xcoords(xind_last);
   full_x  = left_x + right_x;
   half_x  = full_x/2;
   shift_x = half_x - left_x;
  
   yind_first = find(y_array, 1, 'first');
   yind_last  = find(y_array, 1, 'last');
   
   bot_y   = ycoords(yind_first) - ycoords(1);
   top_y   = ycoords(end) - ycoords(yind_last);
   full_y  = bot_y + top_y;
   half_y  = full_y/2;
   shift_y = half_y - bot_y;
   
   xcoords = xcoords + shift_x;
   ycoords = ycoords + shift_y;
